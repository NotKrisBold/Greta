...
with open("whatChatbotReceives.txt", mode="w") as f:
    json.dump(productProcess, f, indent=4)
...
if not st.session_state["messages"]:
    ...
    This table lists for each parameter its unit of measure if present, current value representing the selected option, and, if available, a list of other options to consider.
...
if analyze_table:
    prompt = f'''
    Really carefully analyze the product processes table and provide suggestions to maximize the product sustainability, take as much time as you need.
    Remember that this table presents each parameter along with its unit of measure if present, current value representing the selected option, and, if available, a list of other options to consider.
